...
export const getInstruments = async (page = 0, size = 5, sort = 'name,asc') => {
  try {
    const response = await axios.get(
      `${BASE_URL}/instruments?page=${page}&size=${size}&sort=${sort}`
    )
...
export const createInstrument = async (formData) => {
  try {
    const response = await axios.post(`${BASE_URL}/instruments`, formData, {
      headers: {
...
export const updateInstrument = async (payload) => {
  try {
    const response = await axios.put(`${BASE_URL}/instruments`, payload)
    return response.data
  } catch (error) {
    handleApiError(error)
  }
...
export const deleteInstrument = async (idInstrument) => {
  try {
    const response = await axios.delete(
      `${BASE_URL}/instruments/${idInstrument}`
    )
    return response.data
  } catch (error) {
    handleApiError(error)
