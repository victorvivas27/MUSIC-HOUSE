...
export const FeedbackAdmin = () => {
  ...
  const {
    ...
    safePage
  } = usePaginationControl(state.feedback.totalElements)
  ...
  const fetchData = async (
    ...
    try {
      const data = await getAllFeedback(pageToUse, sizeToUse, sort)
      dispatch({ type: actions.SET_FEEDBACK, payload: data.result })
    } catch {
      dispatch({
  ...
  const rows = Array.isArray(state.feedback.content)
  ...
  const handleRequestSort = (event, property) => {
    const column = headCellsFeedback.find((col) => col.id === property)
  ...
  const handleConfirmDelete = async (idFeedbck = null) => {
    const selectedIds = idFeedbck ? [idFeedbck] : selected
    if (selectedIds.length === 0) {
      showError('Error', 'No hay feedback seleccionadas para eliminar.')
      return
    }

    const isConfirmed = await showConfirm(
      `¿Eliminar ${selectedIds.length} feedback(s)?`,
    ...
    try {
      ...
      showSuccess(
        '¡Eliminado(s)!',
        `${selectedIds.length} feedback eliminada(s) correctamente.`
      )
      setSelected([])
        ...
        >
          ...
          <TableContainer>
            ...
            >
              ...
              <TableBody>
                {rows.map((row, index) => {
                  const isItemSelected = isSelected(row.idFeedbck, selected)
                  ...
                  return (
                    <TableRow
                      ...
                      sx={{ cursor: 'pointer' }}
                      onClick={(event) => handleClick(event, row.idFeedback)}
                    >
                      ...
                      <TableCell align="left">
                        ...
                        >
                          ...
                          <Tooltip title="Eliminar">
                            <IconButton
                              onClick={(event) => {
                                handleConfirmDelete(row.idFeedback)
                                event.stopPropagation()
                              }}
                            >
                              <DeleteIcon
